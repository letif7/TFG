...
def extract_backbone_atoms(pdb_file):
    ...
    model = structure[0]
    for chain in model:
        # Iterar sobre todos los residuos en cada cadena
        for residue in chain:
            # Verificar si el residuo es un aminoácido
            if residue.get_id()[0] == ' ' and residue.get_resname() in ['ALA', 'CYS', 'ASP', 'GLU', 'PHE', 'GLY', 'HIS', 'ILE', 'LYS', 'LEU', 'MET', 'ASN', 'PRO', 'GLN', 'ARG', 'SER', 'THR', 'VAL', 'TRP', 'TYR']:
                # Iterar sobre todos los átomos en cada residuo
                for atom in residue:
                    # Verificar si el átomo es parte del backbone (N, CA, C, O)
                    atom_name = atom.get_name()
                    if atom_name in ['N', 'CA', 'C', 'O']:
                        # Guardar las coordenadas del átomo
