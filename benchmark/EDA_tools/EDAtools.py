...
"determina Jeffreys distance"
# Jeffreys distance
def jeffrey_distnce(p1,p2):
    total_fitness = sum(p1)
    p11= [iji / total_fitness for iji in p1]
    
    total_fitness = sum(p2)
    p22 = [iji / total_fitness for iji in p2]
    salida=0
    for i in range(len(p1)):
        salida=salida+(p11[i]-p22[i])*np.log(p11[i]/p22[i])
    return(salida)
...
def entropia_descrip_val(descriptor_ref, descriptor_temp,lugar):
    ...
    kl_divergence = jeffrey_distnce(prob1, prob2)
...
def entropia_descrip_prob(descriptor_ref, descriptor_temp,lugar):
    ...
    kl_divergence = jeffrey_distnce(data1, data2)
    return kl_divergence
...
def actualiza_individuo_red(poblacion,all_best_execution,tot_act_red,n):
    for elementos in range(tot_act_red):
        for elementos1 in range(n):
            if elementos1==0:
                #print([elementos,elementos1,len(all_best_execution)])
